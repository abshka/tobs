...
class CacheManager:
    def __init__(self, cache_path: Path):
        """Инициализирует менеджер кэша для централизованной работы с файлом кэша."""
    ...
    async def load_cache(self):
        """Асинхронно загружает данные кэша из JSON-файла."""
    ...
    def _get_default_entity_cache(self) -> Dict[str, Any]:
        """Возвращает структуру по умолчанию для новой сущности в кэше."""
        return {"processed_messages": {}, "last_id": None, "title": "Unknown", "type": "unknown"}

    async def save_cache(self):
        """Сохраняет текущее состояние кэша в JSON-файл, если он был изменен."""
    ...
    async def schedule_background_save(self):
        """Планирует операцию сохранения кэша, если это необходимо и она еще не запущена."""
    ...
    async def _with_entity_data(self, entity_id: Union[str, int], operation: Callable, modify: bool = False):
        """Выполняет операцию над данными сущности с блокировкой."""
    ...
    async def is_processed(self, message_id: int, entity_id: Union[str, int]) -> bool:
        """Проверяет, был ли обработан ID сообщения для указанной сущности."""
