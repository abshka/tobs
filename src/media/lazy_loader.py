...
class LazyMediaMetadata:
    ...
    def __post_init__(self):
        ...
        if self.lazy_load_token is None:
            self.lazy_load_token = f"{self.entity_id}_{self.message_id}_{int(self.created_at)}"
...
class LazyMediaLoader:
    ...
    async def store_media_metadata(
        ...
        try:
            ...
            logger.debug(f"Stored lazy metadata for message {message.id}: {metadata.file_name}")
            return metadata
            
        except Exception as e:
            logger.error(f"Failed to store media metadata for message {message.id}: {e}")
            return None
    ...
    async def download_lazy_media(
        ...
        try:
            ...
            logger.info(f"Would download {metadata.file_name} ({metadata.file_size} bytes)")
    ...
    async def _save_metadata(self, metadata: LazyMediaMetadata):
        ...
        try:
            ...
            logger.error(f"Failed to save metadata for token {metadata.lazy_load_token}: {e}")
