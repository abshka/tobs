...
class TestExportMetrics:
    ...
    def test_record_export_complete_failure(self):
        ...
        collector.record_export_complete(
            "test_chat", success=False, error="Connection timeout", error_count=3
        )
...
class TestWorkerMetrics:
    ...
    def test_record_worker_task_failure(self):
        ...
        collector.record_worker_task(
            "worker_0", success=False, error="Processing failed", duration_ms=500
        )

        assert collector.workers["worker_0"].tasks_failed == 1
        assert len(collector.workers["worker_0"].error_history) == 1
        assert (
            collector.workers["worker_0"].error_history[0]["error"]
            == "Processing failed"
        )
    ...
    def test_worker_error_history_limit(self):
        ...
        # Add 15 errors
        for i in range(15):
            collector.record_worker_task("worker_0", success=False, error=f"Error {i}")
...
class TestJSONExport:
    ...
    def test_export_json(self):
        ...
        collector.record_export_start("test_channel", "channel")
        collector.record_export_complete(
            "test_channel", message_count=100, success=True
        )
    ...
    def test_export_json_with_complex_data(self):
        ...
        for j in range(3):
            for k in range(5):
                collector.record_worker_task(f"worker_{j}", success=(k < 3))
...
class TestExportsByType:
    ...
    def test_get_exports_by_type(self):
        ...
        for target_type in ["channel", "chat", "forum"]:
            for i in range(2):
                collector.record_export_start(f"{target_type}_{i}", target_type)
                collector.record_export_complete(
                    f"{target_type}_{i}", success=(i == 0), message_count=100
                )
